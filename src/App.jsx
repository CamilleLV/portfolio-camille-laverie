    ...
    <div className="min-h-screen bg-dash-bg text-dash-text-main selection:bg-dash-accent selection:text-white font-sans">
      ...
      <nav className="fixed top-0 w-full z-50 bg-dash-card/80 backdrop-blur-md border-b border-dash-border">
        <div className="max-w-xl mx-auto px-6 h-16 flex items-center justify-between">
          <span className="font-bold flex items-center gap-2 text-dash-accent uppercase tracking-tighter">
            <LeafIcon size={18} /> Camille.L
          </span>
          <div className="flex gap-6 text-[10px] font-bold text-dash-text-muted uppercase tracking-widest">
      ...
      <main className="max-w-xl mx-auto px-6 pt-32 pb-20 space-y-32">
        ...
        <section id="intro" className="space-y-8">
          ...
          <div className="flex flex-wrap gap-4 pt-4">
            ...
            <a href="#contact" className="flex items-center gap-2 bg-dash-card border border-dash-border hover:border-dash-accent text-dash-text-main px-8 py-4 rounded-2xl font-bold transition-all active:scale-95">
        ...
        <section id="projets" className="space-y-12">
          <div className="flex items-center gap-4">
            ...
            <div className="h-[2px] flex-1 bg-dash-border"></div>
          </div>
              ...
              <div key={index} className="group space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    {project.github && (
                      <a href={project.github} className="p-2 bg-dash-card border border-dash-border rounded-full text-dash-text-muted hover:text-dash-accent hover:border-dash-accent transition-all">
                        <GithubIcon size={20} />
                      </a>
                    )}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map(tag => (
                      <span key={tag} className="text-[10px] font-black uppercase tracking-tighter bg-dash-accent/5 border border-dash-accent/10 px-3 py-1 rounded-full text-dash-accent">
                        {tag}
                      </span>
                    ))}
                  </div>
                  <p className="text-dash-text-muted text-base leading-relaxed font-medium">
                    {project.description}
                  </p>
                </div>
              </div>
          ...
          <div className="bg-dash-text-main rounded-[2.5rem] p-10 text-white space-y-8 relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col sm:flex-row gap-6 items-start sm:items-center justify-between border-t border-white/10 pt-8">
              ...
              <div className="flex gap-4">
                <a href={data.profile.linkedin} className="p-4 bg-white/5 rounded-2xl hover:bg-dash-accent transition-all">
                  <LinkedinIcon size={24} />
                </a>
                <a href={data.profile.github} className="p-4 bg-white/5 rounded-2xl hover:bg-dash-accent transition-all">
                  <GithubIcon size={24} />
                </a>
              </div>
            </div>
      ...
      <footer className="text-center py-12 text-[10px] text-dash-text-muted font-bold uppercase tracking-[0.2em] border-t border-dash-border">
        © 2026 — Camille Laverie — Built with Carbon Logic
      </footer>
    </div>
  );
}
